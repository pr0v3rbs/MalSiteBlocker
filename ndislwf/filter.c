...
--*/
{
    ...
    do
    {
        ...
        InitializeUrlListTable();
        InitializeWhiteListTable();
        Status = CreateDeleteUrlListThread();

        if (Status != NDIS_STATUS_SUCCESS)
        {
            DEBUGP(DL_WARN, "Create response thread failed.\n");
            break;
        }
...
--*/
{
    ...
    //
    gFilterEnable = FALSE;
    UninitializeWhiteListTable();
    UninitializeUrlListTable();
    TerminateDeleteUrlListThread();
       ...
       DispatchLevel = NDIS_TEST_SEND_AT_DISPATCH_LEVEL(SendFlags);
        ...
        if (gFilterEnable)
        {
                ...
                NetBufferLists->FirstNetBuffer->MdlChain->Next != NULL) // HTTP data가 존재하는지 확인.
            {
                ...
                if (IsScanningUrlPort(srcPort, &urlInfo))
                {
                    // 해당 커넥션이 아직 분석되지 않았다면
